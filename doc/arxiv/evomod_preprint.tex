...
Phylogenetic trees are arguably the most common representations of evolutionary processes, including the characterization of pathogen spread, the relation between different species and the evolution of cancers.
...
We explain the theoretical properties of this procedure and study different evolutionary behaviors by performing statistical analysis in these projections.
...
A series of three biologically motivated examples are presented, spanning cancer and infectious disease.
The first examines a link between geometric information and the histologic grade in relapsed gliomas, where longer relapse branches were specific to high grade glioma.
The second concerns genetic stability of xenograft models of cancer, where heterogeneity at the single cell level increased with later mouse passages.
...
This provides an estimate of influenza vaccine efficacy using genomic data. 
...
%
%To assess how different sets of trees can be distinguished in the moduli space, we created synthetic data sets.  For generating each cluster of trees,  a single topology was randomly chosen and branch lengths were drawn from a Dirichlet distribution of $2l-3$ number of parameters, where $l$ is the number of leaves. As such, trees obtained in this fashion, are necessarily unit-norm. Figure \ref{fig:simulation} shows an example of six tree clusters in $\Sigma_5$.  $k$-means clusterings were evaluated over a range of $k$ values using the Davies-Bouldin (DB) index. The DB index correctly identifies the number of clusters. 
%
%\begin{figure}[t!]
%    \centering
%    \includegraphics[height=5in]{../figures/simulation.pdf}
%    \caption{{\bf Model selection for clustering.} Well resolved groups of phylogenetic trees were generated at different numbers of leaves. Three groups of 3-leaved trees, four groups of 4-leaved trees, and six groups of 5-leaved trees were generated with branch lengths drawn from a Dirichlet distribution. We evaluate the quality of $k$-means clusterings over a range of $k$ values, using the elbow of the Davies-Bouldin index as a measure of optimal model complexity. (Top) A visualization of the six groups of 5-leaved trees to be clustered. (Bottom) DB-index shows a sharp inflection point at the correct values of $k$ in all three spaces.}
%    \label{fig:simulation}
%\end{figure}
%
%\subsection{Birth-Death Processes}
%
%To gain a better intuition for the behavior of longitudinal phylogenetic data in our moduli spaces, we turn to stochastic simulations of clonally evolving populations. The advantage of these simulations is that they provide  better understanding of biological interpretable behaviors in more realistic situations.
%Here we simulate clonal evolution at varying degrees of selection strength and mutation rate.
...
%
%We leverage the package OncoSimulR \cite{}, which implements an forward-time evolutionary simulation wherein mutations are designated either driver or passenger events.
%This terminology simply refers to whether the impact of a mutation increases or decreases the birth rate of a genome.
%The magnitudes of fitness effects for driver and passenger mutations are free parameters, as are the mutation rate, numbers of genomic positions at which driver or passenger events can occur.
%Population bottlenecks are crudely implemented by seeding a second simulations with a single member drawn from a time slice of the first simulation.
%Bulk sequencing of the population can be approximated by sampling only the most prevalent genotype at a given time slice.
...
Using public data from chronic lymphocytic leukemia and relapsed glioma, we highlight clinical correlates to the spatial distribution of patients in the moduli space.
...
The approach we present here is based on subsampling of a larger structure and performing statistical analysis in lower dimensional projections.
By decomposing a large tree into distributions of smaller trees we derive two principal advantages.
First, one can easily visualize, interpret, and statistically quanitfy different types of evolutionary relationships.
...
These continuous antigenic changes, often referred as antigenic drift, can lead to failures in vaccine efficacy.
...
For this purpose we study the recent history of influenza A H3N2 using 1,089 sequences of hemmagglutinin collected in the United States between 1993 and 2016 (Figure \ref{fig:flu_HA_sampling}).
Genomic data was downloaded from the GISAID EpiFlu database (www.gisaid.org), and aligned with MUSCLE using default parameters.~\cite{edgar2004muscle}
...
Using 1,089 full length sequences of hemmagglutinin collected in New York state between 1993 and 2016, we relate HA sequences from one season to those from the preceeding ones.
...
The case of length five windows is illustrated in \ref{fig:flu_quint}, where we superimpose each temporal slice onto the same moduli space.
The vast majority of subsampled trees show linear evolution of HA from one season to another, indicating genetic drift as the virus's dominant evolutionary process.
...
It is clear, both from the left and right panels, that lower variance in a temporal window predicts increased future vaccine effectiveness, with a Spearman correlation of -0.52 and p-value of 0.02. The lone outlier season came in 1997-1998. \cite{gupta2006quantifying}, when the vaccine efficacy was lower than expected. In this season the dominant circulating strain was A/Sydney/5/97 while the vaccine strain was A/Wuhan/359/95. 
The analysis can be carried out with length-4 or length-5 windows to yield a similar result. (See supplementary material)

The correlation between variance of tree distributions and vaccine efficacy allows to estimate the influenza vaccine efficacy for future seasons based on genomic data. In particular, for the 2016-2017 season, the variance in tree space was 12.77, corresponding to an approximate efficacy of 36\%. 

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{../figures/influenza_VE.png}
    \caption{{\bf Diversity in recent circulating HA predicts vaccine failure.} Negative correlation observed between vaccine efficacy in season $(t, t+1)$ and the variance in trees generated from seasons $(t-1,t), (t-2. t-1), (t-3,t-2)$.}
    \label{fig:flu_VE}
\end{figure} 
...
We show how to reduce complex trees to a large collections of small trees amenable to statistical approaches.
...
In particular, we have observed that the distributions of sample trees from genomic data can inform influenza vaccine efficacy in future seasons.
The next step will be to find more expressive summarizations of distributions for framing prediction tasks.
As sampling populations of viruses and cells become more extensive, we anticipate many problem statements amenable to the moduli framework.
