...
Is sum, we present here a formalism that links genomic data of influenza A  to important clinical observables, vaccine selection and efficacy.
...
We say that a triangle $T$ in $M$ satisfies the $\CAT(0)$ inequality if for every pair of points $z$ and $\tilde{z}$, we have $d(r,z) \leq d(\tilde{r},\tilde{z})$.
 % $x$ and $y$ in $T$ and comparison points $\tilde{x}$ and $\tilde{y}$ on $\tilde{T}$, we have $d(x,y) \leq d(\tilde{x}, \tilde{y})$.
