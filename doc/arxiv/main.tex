...
\begin{theorem}
For $T,T' \in \aE$ such that $d_{\aE}(T,T') \leq \epsilon$, then for any $S \in \{1,\ldots,m\}$,
\[
d_{\aE}(\Psi_S(T), \Psi_S(T')) \leq \epsilon.
\]
...
Let $A$ and $B$ be subsets of $\aE$ such that $|A| = |B|$.  
Define the matching distance between $A$ and $B$ to be 
\[
d_M(A,B) = \min_{\phi} \max_{a \in A} d_{\aE}(a,\phi(b)),
\]
...
For $T, T' \in \aE$, 
\[
d_{\aE}(T,T') \leq d_M(\Psi_C(T), \Psi_C(T')).
\]
\end{theorem}
