...
The next two theorems provide the theoretical support for the use of tree dimensionality reduction.
...
The strategy of proof is simply to observe that a path $\gamma$ from $T$ to $T'$ in $\aE_m$ induces a path $\Psi_S(\gamma)$ from $\Psi_S(T)$ to $\Psi_S(T')$ in $\Sigma_{|S|}$, with length of the induced path bounded by length of $\gamma$.

{\bf Question: maybe this should go above?}
To be specific, we argue as follows.
The distance $d_{\aE_m}(T,T')$ is realized by a path $\gamma$, which we can express in terms of a finite list of tree rotations $(R_1, R_2, \ldots, R_k)$ along with a scaling factor for each edge.
Now, fix a subset $S \in \{1,\ldots,m\}$.
We will construct a path from $\Psi_S(T)$ to $\Psi_S(T')$ in $\Sigma_{|S|}$ as follows.
...
If the rotation is completely contained, then we can perform the corresponding rotation on $\Psi_S(T)$ and scale the edges as specified by $\gamma$.
If the rotation is not completely contained, we scale the edges incident to the rotation vertex as specified by $\gamma$ and move on to the next rotation in $\gamma$.
This forms a path $\gamma'$ contained in $\Sigma_{|S|}$.

The result follows from two assertions.
...
{\bf Insert remarks about not being able to detect symmetries of trees this way.}
